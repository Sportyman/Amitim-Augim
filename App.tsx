...
const App: React.FC = () => {
  ...
  const uniqueAgeGroups = useMemo(() => {
    const ageGroups = activities.map(a => a.ageGroup.trim()).filter(Boolean);
  ...
  const handleAgeGroupToggle = (ageGroup: string) => {
    setSelectedAgeGroups(prev => prev.includes(ageGroup) ? prev.filter(ag => ag !== ageGroup) : [...prev, ageGroup]);
  };
  ...
  const filteredActivities = useMemo(() => {
    return activities.filter((activity) => {
      ...
      const ageGroupMatch = selectedAgeGroups.length === 0 || selectedAgeGroups.includes(activity.ageGroup);

      return categoryMatch && termMatch && cityMatch && locationMatch && ageGroupMatch;
    });
  }, [selectedCategories, searchTerm, relatedKeywords, activities, selectedCities, selectedLocations, selectedAgeGroups]);
