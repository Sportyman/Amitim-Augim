          ...
          <div key={category.id} className="flex flex-col items-center gap-2">
            <button
              ...
              className={`w-20 h-20 sm:w-24 sm:h-24 rounded-full flex items-center justify-center transition-all duration-300 border-2 ${
                isSelected
                  ? 'bg-sky-100 border-sky-500 scale-105'
                  : 'bg-white border-gray-300 hover:border-sky-400'
              }`}
            >
              <Icon className={`w-8 h-8 sm:w-10 sm:h-10 transition-colors duration-300 ${isSelected ? 'text-sky-500' : 'text-gray-500'}`} />
            </button>
            <span className={`text-sm sm:text-base font-semibold ${isSelected ? 'text-sky-600' : 'text-gray-700'}`}>
              {category.name}
            </span>
          </div>
        );
